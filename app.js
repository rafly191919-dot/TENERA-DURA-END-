...
const PAGE_TITLES = {
  dashboardPage:"Dashboard", inputPage:"Input Transaksi", transactionsPage:"Data Transaksi", rekapPage:"Rekap",
  spreadsheetPage:"Spreadsheet", waPage:"Laporan WA", suppliersPage:"Supplier"
};
...
function bindUI() {
  ...
  window.resetRekapFilters = resetRekapFilters;
  window.renderReportPage = renderReportPage;
  window.renderSpreadsheetTable = renderSpreadsheetTable;
  ...
  ["tenera","dura","tanggal","sopir","plat","supplierInput","supplierSelect"].forEach(id => {
    ...
    if (el) {
      if (id === "sopir") {
        el.addEventListener("input", () => { autoFillByDriver(); updateValidationPreview(); });
        el.addEventListener("change", () => { autoFillByDriver(); updateValidationPreview(); });
      } else {
  ...
  ["dashboardStart","dashboardEnd","dashboardSupplierFilter","dashboardDriverFilter","dashboardSort"].forEach(id => addChange(id, renderDashboard));
  ...
  ["waTemplate","waDate","waStart","waEnd","waSupplierFilter","waDriverFilter","waSort"].forEach(id => addChange(id, generateWA));
  ["rekapStart","rekapEnd","rekapSupplierFilter","rekapDriverFilter","rekapSort"].forEach(id => addChange(id, renderReportPage));
...
function setTodayInputs() {
  ...
  ["tanggal","waDate","waStart","waEnd","dashboardStart","dashboardEnd","rekapStart","rekapEnd","sheetStart","sheetEnd"].forEach(id => {
    const el = document.getElementById(id);
...
function refreshCurrentPage() {
  if (currentPageId === "dashboardPage") renderDashboard();
  if (currentPageId === "transactionsPage") renderTransactionsTable();
  if (currentPageId === "rekapPage") renderReportPage();
...
function autoFillByDriver() {
  const driverInput = document.getElementById("sopir");
  if (!driverInput) return;
  const driverName = driverInput.value.trim().toLowerCase();
  if (!driverName) return;
  const matches = transactionsCache
    .filter(r => safeText(r.sopir).trim().toLowerCase() === driverName && (r.supplier || r.plat))
    .sort((a,b) => new Date(b.created_at || b.updated_at || b.tanggal || 0) - new Date(a.created_at || a.updated_at || a.tanggal || 0));
  const latest = matches[0];
  if (!latest) return;
  if (latest.supplier) setSupplierValue(latest.supplier);
  if (latest.plat) document.getElementById("plat").value = latest.plat;
}
...
function refreshQuickLists() {
  ...
  ["dashboardSupplierFilter","txSupplierFilter","sheetSupplierFilter","waSupplierFilter","rekapSupplierFilter"].forEach(id => fillSelect(id, supplierOptions));
  ["dashboardDriverFilter","txDriverFilter","sheetDriverFilter","waDriverFilter","rekapDriverFilter"].forEach(id => fillSelect(id, driverOptions));
}
...
function renderChips(id, values, fieldId) {
  ...
  wrap.querySelectorAll(".chip").forEach(chip => chip.addEventListener("click", () => {
    ...
    if (chip.dataset.field === "sopir") autoFillByDriver();
...
function getDashboardRows() {
  ...
  const supplier = document.getElementById("dashboardSupplierFilter")?.value;
  const driver = document.getElementById("dashboardDriverFilter")?.value;
  const sort = document.getElementById("dashboardSort")?.value || "newest";
  ...
  rows.sort((a,b) => {
    if (sort === "oldest") return new Date(a.created_at || a.updated_at || a.tanggal || 0) - new Date(b.created_at || b.updated_at || b.tanggal || 0);
    if (sort === "tenera_desc") return toNumber(b.persen_tenera) - toNumber(a.persen_tenera);
    if (sort === "tenera_asc") return toNumber(a.persen_tenera) - toNumber(b.persen_tenera);
    if (sort === "dura_desc") return toNumber(b.persen_dura) - toNumber(a.persen_dura);
    if (sort === "dura_asc") return toNumber(a.persen_dura) - toNumber(b.persen_dura);
    if (sort === "total_desc") return toNumber(b.total) - toNumber(a.total);
    if (sort === "total_asc") return toNumber(a.total) - toNumber(b.total);
    return new Date(b.created_at || b.updated_at || b.tanggal || 0) - new Date(a.created_at || a.updated_at || a.tanggal || 0);
  });
  return rows;
}
function resetDashboardFilters() {
  ...
  const ds = document.getElementById("dashboardStart"); if (ds) ds.value = firstDayOfMonth;
  const de = document.getElementById("dashboardEnd"); if (de) de.value = today;
  ["dashboardSupplierFilter","dashboardDriverFilter"].forEach(id => { const el = document.getElementById(id); if (el) el.value = ""; });
  const sort = document.getElementById("dashboardSort"); if (sort) sort.value = "newest";
  renderDashboard();
}
function getTopQualityLabel(rows, field, label) {
  if (!rows.length) return "-";
  const bySupplier = aggregateBy(rows, "supplier").sort((a,b) => b[field] - a[field]);
  const top = bySupplier[0];
  return top ? `${top.name} (${top[field].toFixed(2)}% ${label})` : "-";
}
...
function renderDashboard(rows = null) {
  const selectedRows = rows || getDashboardRows();
  const summary = computeSummary(selectedRows);
  const start = document.getElementById("dashboardStart")?.value || "awal";
  const end = document.getElementById("dashboardEnd")?.value || "akhir";
  const qualityHTML = `
    <div class="card dashboard-extra-card">
      <div class="card-head"><h4>Informasi Tambahan Dashboard</h4></div>
      <div class="metric-grid">
        <div class="metric-card"><div class="label">Periode tampil</div><div class="value small-value">${escapeHtml(start)} s/d ${escapeHtml(end)}</div></div>
        <div class="metric-card"><div class="label">Supplier % Dura tertinggi</div><div class="value small-value">${escapeHtml(getTopQualityLabel(selectedRows, "persenDura", "Dura"))}</div></div>
        <div class="metric-card"><div class="label">Supplier % Tenera tertinggi</div><div class="value small-value">${escapeHtml(getTopQualityLabel(selectedRows, "persenTenera", "Tenera"))}</div></div>
        <div class="metric-card"><div class="label">Data terakhir masuk</div><div class="value small-value">${selectedRows[0] ? escapeHtml(`${selectedRows[0].tanggal || "-"} • ${selectedRows[0].supplier || "-"}`) : "-"}</div></div>
      </div>
    </div>`;
  document.getElementById("dashboardSummary").innerHTML = summaryCardsHTML(summary) + qualityHTML;
...
function getDateRangeForMode() {
  const start = document.getElementById("rekapStart")?.value || formatDateId(new Date());
  const end = document.getElementById("rekapEnd")?.value || start;
  return { start, end, label: `${start} s/d ${end}` };
}
function getRowsByMode() {
  const range = getDateRangeForMode();
  let rows = transactionsCache.filter(r => inRangeByTanggal(r, parseDate(range.start), parseDate(range.end)));
  const supplier = document.getElementById("rekapSupplierFilter")?.value;
  const driver = document.getElementById("rekapDriverFilter")?.value;
  const sort = document.getElementById("rekapSort")?.value || "date_asc";
  if (supplier) rows = rows.filter(r => r.supplier === supplier);
  if (driver) rows = rows.filter(r => r.sopir === driver);
  rows.sort((a,b) => sort === "date_desc" ? safeText(b.tanggal).localeCompare(safeText(a.tanggal)) : safeText(a.tanggal).localeCompare(safeText(b.tanggal)));
  return rows;
}
function resetRekapFilters() {
  ["rekapStart","rekapEnd"].forEach(id => { const el = document.getElementById(id); if (el) el.value = formatDateId(new Date()); });
  ["rekapSupplierFilter","rekapDriverFilter"].forEach(id => { const el = document.getElementById(id); if (el) el.value = ""; });
  const sort = document.getElementById("rekapSort"); if (sort) sort.value = "date_asc";
  renderReportPage();
}
...
function renderReportPage() {
  const rows = getRowsByMode();
  const sum = computeSummary(rows);
  const range = getDateRangeForMode();
  const target = document.getElementById("rekapReport");
  if (!target) return;
...
function getWAFilters() {
  return {
    ...
    sort: document.getElementById("waSort")?.value || "date_asc"
  };
}
function getRowsForWA() {
  ...
  else if (f.date) rows = rows.filter(r => r.tanggal === f.date);
  if (f.supplier) rows = rows.filter(r => r.supplier === f.supplier);
  if (f.driver) rows = rows.filter(r => r.sopir === f.driver);
  rows.sort((a,b) => f.sort === "date_desc" ? safeText(b.tanggal).localeCompare(safeText(a.tanggal)) : safeText(a.tanggal).localeCompare(safeText(b.tanggal)));
  return rows;
}
function generateWA() {
  ...
  const addTanggal = () => {
    text += "====================================\n\nREKAP PER TANGGAL\n\n";
    aggregateBy(rows, "tanggal").sort((a,b) => safeText(a.name).localeCompare(safeText(b.name))).forEach((x,i) => {
      text += `${i+1}. ${x.name}
`;
      text += `   - Transaksi : ${x.jumlahTransaksi}
`;
...
`;
  ...
  else if (f.template === "tanggal") addTanggal();
  else { addTanggal(); addSupplier(); addDriver(); }
...
function restoreExportLogs() {
  const logs = JSON.parse(localStorage.getItem(LOCAL_KEYS.exportLog) || "{}");
  ["rekapExportInfo","spreadsheetExportInfo"].forEach(id => {
    const el = document.getElementById(id);
...
function exportReportExcel() {
  ...
  setExportLog("rekapExportInfo", `Tanggal ${range.label} berhasil diekspor.`);
}
